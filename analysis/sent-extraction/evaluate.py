...
# df_test = pd.read_csv("../twitter-datasets/train_pos.csv")
df = pd.read_csv("../twitter-datasets/train_pos.txt", delimiter='/n', header=None, error_bad_lines=False)
df_test = pd.DataFrame()
df_test.loc[:, "text"] = df.values[:, 0]
df_test.loc[:, "selected_text"] = df.values[:, 0]
df_test.loc[:, "sentiment"] = ["positive"] * len(df.values)
...
final = pd.DataFrame()
final.loc[:, "selected_text"] = final_ouput
final.selected_text = final.selected_text.map(post_process)
pd.DataFrame(final).to_csv("final_test.csv", header=True)
